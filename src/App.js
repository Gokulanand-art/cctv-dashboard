  ...
  return (
    <span style={{ display: "inline-flex", alignItems: "center", gap: 6 }}>
      <span style={{
        ...
        border: isLight ? "1px solid #4a5568" : "none", flexShrink: 0,
  ...
  return (
    <div onClick={onClick} style={{
      ...
      <div style={{ display: "flex", gap: 16, marginTop: 12 }}>
        <div>
          <div style={{ color: "#4a5568", fontSize: 10, marginBottom: 3 }}>UPPER</div>
          <ColorDot color={card.upper_color} />
        </div>
        <div>
          <div style={{ color: "#4a5568", fontSize: 10, marginBottom: 3 }}>LOWER</div>
...
function DetailPanel({ card }) {
  if (!card) return (
    <div style={{
      ...
      justifyContent: "center", height: "100%", color: "#2d3748", gap: 12,
    }}>
    ...
    <div style={{ padding: "24px 28px", height: "100%", overflowY: "auto" }}>
      ...
      {card.notes && card.notes.length > 0 && (
        <div style={{ background: "#1a1200", borderRadius: 10, padding: "14px 16px", border: "1px solid #3d2e00" }}>
          ...
          {card.notes.map((n, i) => (
            <div key={i} style={{ color: "#f6ad55", fontSize: 13 }}>⚑ {n}</div>
          ))}
        </div>
      )}
    </div>
...
function CameraMap({ cards, selected }) {
  return (
    <div style={{ background: "#0d1117", borderRadius: 10, border: "1px solid #1e293b", padding: 16 }}>
      <div style={{ color: "#4a5568", fontSize: 11, letterSpacing: "0.1em", marginBottom: 10 }}>CAMERA VIEW — POSITION MAP</div>
      <div style={{ background: "#080d14", borderRadius: 8, border: "1px solid #1a2535", position: "relative", height: 180, overflow: "hidden" }}>
        {[...Array(4)].map((_, i) => (
...
export default function App() {
  ...
  const loadHistoryFile = async (filename) => {
    try {
      const res = await fetch(`${BACKEND_URL}/history/${filename}`);
      const json = await res.json();
      setData(json);
      setSelected(null);
  ...
  if (loading) return (
    <div style={{
      background: "#060b14", minHeight: "100vh", display: "flex",
      alignItems: "center", justifyContent: "center", flexDirection: "column", gap: 16,
      fontFamily: "'IBM Plex Sans', system-ui, sans-serif",
    }}>
      <div style={{ color: "#4299e1", fontSize: 32 }}>📹</div>
      <div style={{ color: "#4a5568", fontSize: 14 }}>Connecting to CCTV backend...</div>
    </div>
  );
  ...
  if (!data) return (
    <div style={{
      ...
      <div style={{ color: "#e53e3e", fontSize: 32 }}>📭</div>
      <div style={{ color: "#718096", fontSize: 16 }}>No video processed yet</div>
      <div style={{ color: "#4a5568", fontSize: 13 }}>Run the Python script to see results here</div>
      <code style={{ color: "#4299e1", background: "#0d1117", padding: "8px 16px", borderRadius: 8, fontSize: 12 }}>
        python3 smart_cctv_system.py --video your_video.mp4
      </code>
    </div>
  );
  ...
  const avgDuration = cards.length
    ? Math.round(cards.reduce((s, c) => s + c.duration_seconds, 0) / cards.length)
    : 0;
  ...
  return (
    <div style={{
      ...
      fontFamily: "'IBM Plex Sans', 'Segoe UI', system-ui, sans-serif", color: "#e2e8f0",
    }}>
      <style>{`
      ...
      {/* Header */}
      <div style={{
        background: "#080d14", borderBottom: "1px solid #1e293b",
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          ...
          <div>
            <div style={{ color: "#e2e8f0", fontWeight: 700, fontSize: 16 }}>Smart CCTV Person Card System</div>
            <div style={{ color: "#4a5568", fontSize: 12 }}>
              {data.metadata.video_file} · {data.metadata.resolution} · {data.metadata.fps}fps
              {lastUpdated && <span style={{ color: "#2d3748" }}> · updated {lastUpdated}</span>}
            </div>
          </div>
        </div>
        <div style={{ display: "flex", gap: 16, alignItems: "center" }}>
          {[
            { label: "TRACKED", value: data.metadata.total_persons_tracked },
            { label: "CARDS", value: data.metadata.cards_generated },
            { label: "ACTIVE", value: totalActive, accent: true },
            { label: "AVG", value: fmtDuration(avgDuration) },
          ...
          {/* History button */}
          <button onClick={() => { setShowHistory(!showHistory); fetchHistory(); }} style={{
            background: "#0d1117", border: "1px solid #1e293b", color: "#718096",
      ...
      {showHistory && (
        <div style={{
          ...
          padding: "12px 28px", display: "flex", gap: 8, flexWrap: "wrap",
        }}>
          {history.length === 0 ? (
            <span style={{ color: "#4a5568", fontSize: 13 }}>No archived videos yet</span>
          ) : history.map((f) => (
            <button key={f} onClick={() => loadHistoryFile(f)} style={{
              background: "#111827", border: "1px solid #1e293b", color: "#90cdf4",
      ...
      <div style={{ display: "grid", gridTemplateColumns: "340px 1fr 320px", height: `calc(100vh - ${showHistory ? 115 : 65}px)` }}>
        {/* Left: Card list */}
        <div style={{ borderRight: "1px solid #1e293b", display: "flex", flexDirection: "column", overflow: "hidden" }}>
          ...
          <div style={{ flex: 1, overflowY: "auto", padding: "12px 14px", display: "flex", flexDirection: "column", gap: 10 }}>
            {filtered.length === 0 ? (
              ...
              <PersonCard key={card.person_id} card={card}
        ...
        <div style={{ padding: 16, overflowY: "auto", display: "flex", flexDirection: "column", gap: 14 }}>
                ...
                <div key={color} style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 8 }}>
                  ...
                  <div style={{ flex: 1 }}>
                    ...
                    <div style={{ background: "#1a2535", borderRadius: 2, height: 4 }}>
                      <div style={{ width: `${(count / cards.length) * 100}%`, height: "100%", background: COLOR_HEX[color] || "#718096", borderRadius: 2 }} />
                    </div>
                  </div>
                </div>
              ));
          ...
          <div style={{ background: "#0a1a0a", borderRadius: 10, padding: "12px 16px", border: "1px solid #1a3327", textAlign: "center" }}>
            <div style={{ color: "#48bb78", fontSize: 11 }}>● Auto-refreshes every 10 seconds</div>
            <div style={{ color: "#2f6846", fontSize: 11, marginTop: 4 }}>New video results appear automatically</div>
          </div>
        </div>
      </div>
    </div>
